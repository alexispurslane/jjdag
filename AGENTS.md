...
- **tmux**: Use tmux for interactive testing of TUI workflows. Capture screen via `tmux capture-pane -t <name> -p` rather than tail (avoids truncating text prompts):
  - Create session: `tmux new-session -d -s <name>`
  - Send keys: `tmux send-keys -t <name> "<keys>"`
  - Capture screen: `tmux capture-pane -t <name> -p`
